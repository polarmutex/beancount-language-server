...
pub(crate) fn hover(
    ...
    let account_node = find_account_node(node);
    ...
    let hover_text = format_hover_text(&account_name, &notes);
    let range = Range {
...
fn find_account_node(node: tree_sitter::Node) -> Option<tree_sitter::Node> {
...
fn format_hover_text(account: &str, notes: &[String]) -> String {
    if notes.len() == 1 {
        format!("**{}**\n\n{}", account, notes[0])
    } else {
        let mut text = format!("**{}**\n\nNotes:\n", account);
