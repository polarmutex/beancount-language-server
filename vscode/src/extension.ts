...
async function get_server_path(
  ...
  const resolvers: Array<{
    ...
    {
      reason: "python: active environment",
      resolver: () => find_in_selected_python_env(),
    },
    {
...
async function find_in_selected_python_env(): Promise<string | undefined> {
  const resource =
  ...
  try {
    ...
    if (environmentPath?.path) {
      ...
      const executableUri = resolved?.executable?.uri;
      if (executableUri?.fsPath) {
        return find_in_dir(path.dirname(executableUri.fsPath));
      }
    }
  } catch (_) {
    return undefined;
